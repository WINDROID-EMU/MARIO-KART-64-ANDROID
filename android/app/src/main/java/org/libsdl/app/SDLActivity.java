...
public class SDLActivity extends Activity implements View.OnSystemUiVisibilityChangeListener {
    ...
    private class VirtualGamepadView extends View {
        ...
        private Paint cStrokePaint;
        private Paint cFillPaint;
        ...
        private int mOpacityPercent = 70;
        private float mSizeScale = 1.0f;
        private boolean mShowCButtons = true;
        private boolean mShowExtraZ = true;
        private boolean mFloatingAnalog = true;

        public void reloadConfig() {
            try {
                SharedPreferences prefs = getContext().getSharedPreferences(ConfigActivity.PREFS_NAME, Context.MODE_PRIVATE);
                mOpacityPercent = prefs.getInt("ctrl_opacity", 70);
                if (mOpacityPercent < 20) mOpacityPercent = 20;

                int sizeIdx = prefs.getInt("ctrl_size_idx", 1);
                float[] scales = {0.8f, 1.0f, 1.2f, 1.4f};
                if (sizeIdx >= 0 && sizeIdx < scales.length) mSizeScale = scales[sizeIdx];
                else mSizeScale = 1.0f;

                mShowCButtons = prefs.getBoolean("ctrl_c_buttons", true);
                mShowExtraZ = prefs.getBoolean("ctrl_extra_z", true);
                mFloatingAnalog = prefs.getBoolean("ctrl_floating_analog", true);

                int alpha = (int) (mOpacityPercent * 2.55f);
                int colorWhite = (alpha << 24) | 0x00FFFFFF;
                int colorGold = (alpha << 24) | 0x00FFD700;

                strokePaint.setColor(colorWhite);
                fillPaint.setColor(colorWhite);
                textPaint.setColor(colorWhite);
                cStrokePaint.setColor(colorGold);
                cFillPaint.setColor(colorGold);
                cTextPaint.setColor(colorGold);
                ...
                Log.e(TAG, "Error reloading gamepad config", e);
            }
        }
        ...
        public VirtualGamepadView(Context context) {
            ...
            int colorGold = 0xCCFFD700;  // Amber / Gold for C-buttons
            ...
            cStrokePaint = new Paint();
            cStrokePaint.setColor(colorGold);
            cStrokePaint.setStyle(Paint.Style.STROKE);
            cStrokePaint.setStrokeWidth(5f);
            cStrokePaint.setAntiAlias(true);

            cFillPaint = new Paint();
            cFillPaint.setColor(colorGold);
            cFillPaint.setStyle(Paint.Style.FILL_AND_STROKE);
            cFillPaint.setAntiAlias(true);

            cTextPaint = new Paint();
            cTextPaint.setColor(colorGold);
            cTextPaint.setTextSize(32f);
            cTextPaint.setTextAlign(Paint.Align.CENTER);
            cTextPaint.setFakeBoldText(true);
        ...
        class ButtonDef {
            String label;
            float x, y, rx, ry; 
            int type; // 0 = circle, 1 = rounded rect, 2 = c-button circle
            int keycode;
            ...
            ButtonDef(String label, float x, float y, float r, int type, int keycode) {
                this.label = label; this.x = x; this.y = y; this.rx = r; this.ry = r; this.type = type; this.keycode = keycode;
            }
        ...
        protected void onSizeChanged(int w, int h, int oldw, int oldh) {
            ...
            float bw = (minDim / 13f) * mSizeScale;
            if (bw > 115) bw = 115;
            if (bw < 35) bw = 35;

            textPaint.setTextSize(bw * 0.7f);
            cTextPaint.setTextSize(bw * 0.55f);
            strokePaint.setStrokeWidth(bw * 0.08f);
            cStrokePaint.setStrokeWidth(bw * 0.07f);
            ...
            ButtonDef btnA = new ButtonDef("A", w - bw * 2.2f, h - bw * 2.5f, bw * 1.3f, KeyEvent.KEYCODE_SHIFT_LEFT);
            ButtonDef btnB = new ButtonDef("B", w - bw * 4.9f, h - bw * 1.8f, bw * 1.05f, KeyEvent.KEYCODE_CTRL_LEFT);
            ButtonDef btnZRight = new ButtonDef("Z", w - bw * 4.8f, h - bw * 4.2f, bw * 1.05f, KeyEvent.KEYCODE_Z);
            ButtonDef btnR = new ButtonDef("R", w - bw * 2.6f, bw * 1.2f, bw * 1.8f, bw * 0.65f, 1, KeyEvent.KEYCODE_SPACE);
            ButtonDef btnL = new ButtonDef("L", bw * 2.6f, bw * 1.2f, bw * 1.8f, bw * 0.65f, 1, KeyEvent.KEYCODE_Q);
            
            ButtonDef btnZLeft = new ButtonDef("Z", bw * 6.5f, bw * 1.2f, bw * 1.6f, bw * 0.65f, 1, KeyEvent.KEYCODE_Z);
            btnZLeft.visible = mShowExtraZ;

            ButtonDef btnCUp = new ButtonDef("C^", w - bw * 8.2f, bw * 1.8f, bw * 0.65f, 2, KeyEvent.KEYCODE_T);
            ButtonDef btnCDown = new ButtonDef("Cv", w - bw * 8.2f, bw * 3.8f, bw * 0.65f, 2, KeyEvent.KEYCODE_G);
            ButtonDef btnCLeft = new ButtonDef("C<", w - bw * 9.3f, bw * 2.8f, bw * 0.65f, 2, KeyEvent.KEYCODE_F);
            ButtonDef btnCRight = new ButtonDef("C>", w - bw * 7.1f, bw * 2.8f, bw * 0.65f, 2, KeyEvent.KEYCODE_H);

            btnCUp.visible = mShowCButtons;
            btnCDown.visible = mShowCButtons;
            btnCLeft.visible = mShowCButtons;
            btnCRight.visible = mShowCButtons;

            ButtonDef btnStart = new ButtonDef("START", w / 2f, h - bw * 1.2f, bw * 1.6f, bw * 0.55f, 1, KeyEvent.KEYCODE_ENTER);

            buttons = new ButtonDef[] {
                btnA, btnB, btnZRight, btnR, btnL, btnZLeft,
                btnCUp, btnCDown, btnCLeft, btnCRight,
                btnStart,
                dpadUp, dpadDown, dpadLeft, dpadRight
            };
            
            toggleButton = new ButtonDef("☰", w / 2f, bw * 1.2f, bw * 0.55f, 0);
        }
        ...
        protected void onDraw(Canvas canvas) {
            ...
            // Draw Analog Stick Base & Ring
            Paint stickBasePaint = new Paint(fillPaint);
            stickBasePaint.setStyle(Paint.Style.STROKE);
            stickBasePaint.setStrokeWidth(strokePaint.getStrokeWidth());
            stickBasePaint.setColor(0x88FFFFFF);
            canvas.drawCircle(currentDpadX, currentDpadY, dpadR, stickBasePaint);
            ...
            knobPaint.setColor(analogPressed ? 0xCCFFFFFF : 0x77FFFFFF);
            canvas.drawCircle(analogX, analogY, dpadR * 0.45f, knobPaint);
            
            Paint knobStroke = new Paint(strokePaint);
            knobStroke.setColor(0xEEFFFFFF);
            canvas.drawCircle(analogX, analogY, dpadR * 0.45f, knobStroke);
            
            for (ButtonDef b : buttons) {
                if (b == dpadUp || b == dpadDown || b == dpadLeft || b == dpadRight || !b.visible) continue;
                ...
                if (b.type == 0 || b.type == 2) {
                    canvas.drawCircle(b.x, b.y, b.rx, curPaint);
                } else if (b.type == 1) {
                    android.graphics.RectF rect = new android.graphics.RectF(b.x - b.rx, b.y - b.ry, b.x + b.rx, b.y + b.ry);
                    canvas.drawRoundRect(rect, b.ry, b.ry, curPaint);
                }
                
                float oldSize = curTextPaint.getTextSize();
                if (b.type == 1) {
                    curTextPaint.setTextSize(oldSize * 0.65f);
                }
                float textY = b.y - ((curTextPaint.descent() + curTextPaint.ascent()) / 2);
                canvas.drawText(b.label, b.x, textY, curTextPaint);
                if (b.type == 1) {
        ...
        private boolean isTouchOverButton(float x, float y) {
            ...
            if (buttons != null) {
                for (int j = 0; j < buttons.length - 4; j++) { // exclude DPAD defs
                    ...
                    if (b.type == 0 || b.type == 2) {
                        ...
                        if (Math.abs(bx) <= b.rx * 1.4f && Math.abs(by) <= b.ry * 1.8f) {
                            return true;
                        }
                    }
                }
        ...
        public boolean onTouchEvent(MotionEvent event) {
            ...
            // Handle touch down for floating analog stick
            if (controlsVisible && (action == MotionEvent.ACTION_DOWN || action == MotionEvent.ACTION_POINTER_DOWN)) {
                if (actionId != togglePointerId) {
                    float tx = event.getX(actionIndex);
                    float ty = event.getY(actionIndex);
                    // Left half of screen and not over top-left buttons
                    if (tx < getWidth() * 0.45f && ty > getHeight() * 0.25f && !isTouchOverButton(tx, ty)) {
            ...
            for (int i = 0; i < pointerCount; i++) {
                ...
                if (controlsVisible) {
                    ...
                    if (id == analogPointerId) {
                        ...
                        currentFingerX = dx;
                        currentFingerY = dy;
                        if (dy < -dpadR * 0.25f) nextState[dpadUpIdx] = true;    // UP
                        if (dy > dpadR * 0.25f) nextState[dpadDownIdx] = true;  // DOWN
                        if (dx < -dpadR * 0.25f) nextState[dpadLeftIdx] = true;  // LEFT
                        if (dx > dpadR * 0.25f) nextState[dpadRightIdx] = true; // RIGHT
                    }
                    ...
                    for (int j = 0; j < buttons.length - 4; j++) {
                        ...
                        float by = y - b.y;
                        if (b.type == 0 || b.type == 2) {
                            if (bx * bx + by * by <= (b.rx * 1.6f) * (b.rx * 1.6f)) {
                                nextState[j] = true;
                            }
                        } else if (b.type == 1) {
                            if (Math.abs(bx) <= b.rx * 1.3f && Math.abs(by) <= b.ry * 1.6f) {
                                nextState[j] = true;
                            }
                        }
                    }
            ...
            for (int i = 0; i < buttons.length; i++) {
                if (buttons[i].pressed != nextState[i]) {
                    ...
                    changed = true;
                    if (nextState[i]) {
                        SDLActivity.onNativeKeyDown(buttons[i].keycode);
                    } else {
                        SDLActivity.onNativeKeyUp(buttons[i].keycode);
                    }
                }
            }
...
*/
    ...
    protected static VirtualGamepadView mGamepad;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mGamepad = new VirtualGamepadView(this);
        RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
        mLayout.addView(mGamepad, params);
    ...
    private boolean hasStoragePermission() {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.R) {
            ...
            return true; // Below M: permissions granted at install time
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == STORAGE_PERMISSION_REQUEST_CODE) {
            if (hasStoragePermission()) {
                ...
                Log.e(TAG, "Storage permission denied - game cannot load assets!");
    ...
    private void checkAndExtractAssets() {
        java.io.File mk64Dir = new java.io.File(Environment.getExternalStorageDirectory(), "MK64");
        java.io.File mk64Otr = new java.io.File(mk64Dir, "mk64.o2r");

        if (!mk64Dir.exists() || !mk64Otr.exists()) {
            new AlertDialog.Builder(this)
                    .setTitle("Configurando Mario Kart 64")
                    .setMessage("Os arquivos do jogo precisam ser extraídos (~1 minuto). Pressione OK para começar.")
                    .setCancelable(false)
                    .setPositiveButton("OK", (dialog, which) -> {
                        runOnUiThread(() -> Toast.makeText(this, "Extraindo arquivos...", Toast.LENGTH_LONG).show());
                        Executors.newSingleThreadExecutor().execute(() -> {
                            extractAssetsToMK64Dir(mk64Dir);
                            setupLatch.countDown();
                        });
                    })
                    .show();
    ...
    private void extractAssetsToMK64Dir(java.io.File mk64Dir) {
        if (!mk64Dir.exists()) mk64Dir.mkdirs();
        String[] assetsToExtract = {"mk64.o2r", "spaghetti.o2r"};
        for (String assetName : assetsToExtract) {
            try {
                java.io.File destFile = new java.io.File(mk64Dir, assetName);
                if (!destFile.exists()) {
                    Log.v(TAG, "Extracting " + assetName + " to " + destFile.getAbsolutePath());
                    ...
                    out.flush();
                    out.close();
                    Log.v(TAG, "Extraction of " + assetName + " finished.");
                }
